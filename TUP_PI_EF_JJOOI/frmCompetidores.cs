...
namespace TUP_PI_EF_JJOOI
{
    public partial class frmCompetidores : Form
    {
        ...
        private void frmCompetidores_Load(object sender, EventArgs e)
        {
            ...
            this.CargarGrilla(grdCompetidores);
        }

        private void CargarGrilla(DataGridView grilla)
        {
            DataTable tabla = consultarSQL("SELECT distinct c.numero, c.Nombre, d.nombreDisciplina, .fecha  " +
                                             "FROM Competidores c, Disciplinas d");
            grilla.Rows.Clear();
            for (int i = 0; i < tabla.Rows.Count; i++)
            {
                grilla.Rows.Add(tabla.Rows[i][0], tabla.Rows[i][1], tabla.Rows[i][2], tabla.Rows[i][3]);
            }
        }
        ...
        private void grdCompetidores_SelectionChanged(object sender, EventArgs e)
        {
            this.CargarCampos(grdCompetidores.SelectedRows[0].Index);
        }
    }
}
